...
interface MarketplaceCtx {
  ...
  updateKyc: (pan: string, aadhaar: string, address: string, guardian: string) => Promise<{ ok: boolean }>;
...
export function MarketplaceProvider({
  ...
  const updateKyc = useCallback(async (pan: string, aadhaar: string, address: string, guardian: string) => {
    try {
      const r = await fetch("/api/user/kyc", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ pan, aadhaar, address, guardian }),
      });
      const d = await r.json().catch(() => ({}));
      if (r.ok && d.ok) {
        setCurrentUser((u) => (u ? { ...u, pan: d.pan as string, aadhaar: d.aadhaar as string, address: d.address as string, guardian: d.guardian as string } : u));
        return { ok: true };
      }
      return { ok: false };
    } catch {
